...
if __name__ == "__main__":
    ...
    img_enhanced = Path("data/processed/invoice_0000.jpg")

    if not img_enhanced.exists():
        print(f"❌ File not found: {img_enhanced}")
    else:
        print(f"\n--- Running OCR on enhanced image: {img_enhanced.name} ---")

        extracted, avg_conf, fulltext = run_ocr_on_image(img_enhanced)
        ...
        # Save JSON output
        out_json = img_enhanced.with_suffix(".ocr.json")
